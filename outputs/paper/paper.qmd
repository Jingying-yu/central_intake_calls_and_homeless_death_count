...
The original dataset contains three columns: Year of death, Month of death, and Count. The first step of data cleaning, after downloading the data, was to exclude any rows that contained an “unknown” value. Although this action created a source of error in future analyses, this is still a necessary step because we are plotting death counts against a timeline. A date column is then created to combine the information on the year and month of death. The format of this date-class column is “yyyy-mm-dd” where the “dd” is the last day of the month when the death count of the month is finalized. This design allows easier graphing of monthly cumulative counts as discrete variables against time. The date-class column is added to the original dataset by the mutate() function under the tidyverse package [@tidyverse].

Cleaned data is then filtered to include only data from November 2020 to June 2023 and a new column that contains the name of the “Month Year” is added to the dataset (@tbl-death-count). The rationalization behind the filtering will be explained in @sec-data-diff-datasets.
...
knitr::kable(sample_homeless_death_counts, col.names = c("Date Displayed", "Last Day of Month", "Death Count"), align = c("l", "c", "c", "c"))
```



## Using Data from Different Datasets {#sec-data-diff-datasets}
During the data cleaning process, code chunks are written to ensure that both the Central Intake Call Wrap-Up Codes Dataset [@Central_Intake_Call] and the Deaths of People Experiencing Homelessness Dataset [@Homeless_Death_Count] are filtered to include only data between November 1st, 2020 to June 30st, 2023. 

The decision to only include this time period is reached by taking the common time period between the two datasets. In the latest refresh, the Central Intake Call Wrap-Up Codes Dataset [@Central_Intake_Call] begins on November 3rd, 2020 and end on December 31st, 2023; whereas the Deaths of People Experiencing Homelessness Dataset [@Homeless_Death_Count] begins on January of 2017 and ends on June 2023.

The ultimate purpose of taking the common time period is to ensure that we can plot variables of interest in the two datasets onto the same graph.
...
homeless_death_counts <- read_csv(here::here("outputs/data/cleaned_homeless_death_counts.csv"), show_col_types = FALSE)
central_intake_call <- read_csv(here::here("outputs/data/cleaned_central_intake_call.csv"), show_col_types = FALSE)


ggplot(homeless_death_counts, aes(x = Date_month, y = Count)) + 
  geom_bar(stat = "identity", fill = "lightsteelblue", color = "lightsteelblue") +
  geom_line(data = central_intake_call, aes(x = Month, 
    y = Calls_Coded_month/1000)) +
  labs(
    title = "Correlation between number of Calls Coded and Homeless Death Counts",
    x = "Date",
    Y = "Count"
  )
...
```{r}
#| label: fig-referred
#| fig-cap: Visualization to show correlation between number of callers referred to shelter and Homeless Death Counts
#| echo: false
...
ggplot(homeless_death_counts, aes(x = Date_month, y = Count)) + 
  geom_bar(stat = "identity", fill = "lightsteelblue", color = "lightsteelblue") +
  geom_line(data = central_intake_call, aes(x = Month, 
    y = Referral_to_Shelter_month/100)) +
  labs(
    title = "Correlation between # of Callers Referred to Shelter and Homeless Death Counts",
    x = "Date",
    Y = "Count"
  )
#still need to add a legend or label, but I haven't yep figured out how
...
```{r}
#| label: fig-informed
#| fig-cap: Visualization to show correlation between number of callers given information on homelessness prevention and Homeless Death Counts
#| echo: false
...
ggplot(homeless_death_counts, aes(x = Date_month, y = Count)) + 
  geom_bar(stat = "identity", fill = "lightsteelblue", color = "lightsteelblue") +
  geom_line(data = central_intake_call, aes(x = Month, 
    y = Information_Homelessness_and_Prevention_month/100)) +
  labs(
    title = "Correlation between number of Callers given Information on Homelessness Prevention and Homeless Death Counts",
    x = "Date",
    Y = "Count"
  )
#still need to add a legend or label, but I haven't yep figured out how
